...
def main() -> None:
    """Classify sentences in a CSV file using an external LLM."""

    classification_choice = gui.ynbox(
        "Sentiment Classification",
        "Do you want to classify the news using external LLMs?",
    )

    if not classification_choice:
        # User chose not to run the classification
        return
    ...
    df["openai_inferred_sentiment_from_RAG"] = ""
    default_classification_prompt = (
        ". For financial statement above, determine its sentiment (based on your existing knowledge). "
        "Your answer should be either \"negative\" or \"neutral\" or \"positive\""
    )
    classification_prompt = gui.enterbox(
    ...
    try:
        for row_index, row in df.iloc[1:].iterrows():
            ...
            classification_response = external_LLMs.extract_classification(
                target_sentence, classification_prompt
            )
